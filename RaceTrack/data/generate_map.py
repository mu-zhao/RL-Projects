...
class TrackMap:
    ...
    def gen_config(self, start_positions, end_positions):
        ...
        for pose in end_positions:
            if pose[0] < 0 or pose[0] >= len(self.track_list) or \
               pose[1] not in self.track_list[pose[0]]:
                logger.warning(f"point {pose} not in the map region")
            else:
    ...
    def validate(self):
        ...
        while queue:
            ...
            for a, b in [(0, 1), (0, -1), (1, 0), (-1, 0)]:
                ...
                if 0 <= x < len(self.track_list) and y in self.track_list[x]:
